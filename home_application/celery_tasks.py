...
def save_capacity_data(username='admin'):
    ...
    ip = '192.168.24.166'  # 请修改为你需要执行的业务IP(可在蓝鲸配置平台里查询)
    # 作业ID和脚本ID二选一
    ...
    result, job_instance_id = get_job_instance_id(client, biz_id, ip, job_id, script_id, username)
    if result:
        count = 0
        while True:
            is_finish, capacity_data = get_host_capaticy(client, biz_id, job_instance_id, ip, username)
