...
def get_job_instance_id(client, biz_id, ip, job_id=0, script_id=0, username=''):  # 作业ID和脚本ID二选一
    """
    执行Job作业/script脚本
    """
    if job_id:
        ...
        if username:
            kwargs.update({'bk_username': username})
...
def get_host_capaticy(client, biz_id, job_instance_id, ip, username=''):
    """
    获取磁盘容量数据
    """
    kwargs = {
