...
def get_dfinfo_lisir(request):
    try:
        ...
        if result["result"]:
            script_result = get_job_instance_log(client, bk_biz_id, result["data"]["job_instance_id"])
        ...
        if result["data"][0]["is_finished"]:
            if result["data"][0]["status"] == 3:
                ...
                return JsonResponse({
                    ...
                    "data": {"log": log, "time": datetime.datetime.now()}
                })
            else:
                return JsonResponse({
                    "result": "false",
            ...
            time.sleep(3)
            return get_job_instance_log(client, bk_biz_id, job_instance_id)
